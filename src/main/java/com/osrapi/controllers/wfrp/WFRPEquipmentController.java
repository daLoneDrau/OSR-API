...
public class WFRPEquipmentController {
	...
	public List<Resource<WFRPEquipmentEntity>> getAll() {
		Iterator<WFRPEquipmentEntity> iter = repository.findAll()
				.iterator();
		List<Resource<WFRPEquipmentEntity>> resources = new ArrayList<Resource<WFRPEquipmentEntity>>();
		while (iter.hasNext()) {
	...
	public List<Resource<WFRPEquipmentEntity>> getByCostGc(
			@PathVariable final Long costGc) {
	 ...
	 * @return {@link List}<{@link Resource}<{@link WFRPEquipmentEntity}>>
	 */
	@RequestMapping(path = "encumbrance/{encumbrance}", method = RequestMethod.GET)
	public List<Resource<WFRPEquipmentEntity>> getByEncumbrance(
			@PathVariable final Long encumbrance) {
		Iterator<WFRPEquipmentEntity> iter = repository
	...
	private Resource<WFRPEquipmentEntity> getEquipmentResource(
		...
		Resource<WFRPEquipmentEntity> resource = new Resource<WFRPEquipmentEntity>(
				entity);
		// link to entity
	...
	public List<Resource<WFRPEquipmentEntity>> save(
			@RequestBody final List<WFRPEquipmentEntity> entities) {
		List<Resource<WFRPEquipmentEntity>> resources = new ArrayList<Resource<WFRPEquipmentEntity>>();
	...
	public List<Resource<WFRPEquipmentEntity>> save(
			@RequestBody final WFRPEquipmentEntity entity) {
		...
		if (entity.getAvailability() != null
			...
			try {
				...
				if (method != null
					...
					if (field.get(entity.getAvailability()) != null) {
						list = (List<Resource<WFRPEquipmentAvailabilityEntity>>) method
								.invoke(
										WFRPEquipmentAvailabilityController
												.getInstance(),
										(String) field
												.get(entity.getAvailability()));
					}
				}
				if (list == null) {
			...
			if (availability == null) {
				availability = (WFRPEquipmentAvailabilityEntity) ((Resource) WFRPEquipmentAvailabilityController
						.getInstance()
						.save(entity.getAvailability()).get(0)).getContent();
			}
		...
		if (entity.getWeaponQuality() != null
			...
			if (weaponQuality == null) {
				weaponQuality = (WFRPWeaponQualityEntity) ((Resource) WFRPWeaponQualityController
						.getInstance()
						.save(entity.getWeaponQuality()).get(0)).getContent();
			}
