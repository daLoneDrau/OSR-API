...
public class WFRPArmourController {
	...
	public List<Resource<WFRPArmourEntity>> getAll() {
		Iterator<WFRPArmourEntity> iter = repository.findAll()
				.iterator();
		List<Resource<WFRPArmourEntity>> resources = new ArrayList<Resource<WFRPArmourEntity>>();
		while (iter.hasNext()) {
	...
	public List<Resource<WFRPArmourEntity>> save(
			@RequestBody final List<WFRPArmourEntity> entities) {
		List<Resource<WFRPArmourEntity>> resources = new ArrayList<Resource<WFRPArmourEntity>>();
	...
	public List<Resource<WFRPArmourEntity>> save(
			@RequestBody final WFRPArmourEntity entity) {
		if (entity.getCoverage() != null
			...
			for (int i = entity.getCoverage().size() - 1; i >= 0; i--) {
				...
				try {
					...
					if (method != null
						...
						if (field.get(entity.getCoverage().get(i)) != null) {
							list = (List<Resource<WFRPArmourCoverageEntity>>) method
									.invoke(
				...
				if (coverage == null) {
					coverage = (WFRPArmourCoverageEntity) ((Resource) WFRPArmourCoverageController
							.getInstance()
