...
public class WFRPRaceController {
	...
	public List<Resource<WFRPRaceEntity>> getAll() {
		Iterator<WFRPRaceEntity> iter = repository.findAll()
				.iterator();
		List<Resource<WFRPRaceEntity>> resources = new ArrayList<Resource<WFRPRaceEntity>>();
		while (iter.hasNext()) {
	...
	public List<Resource<WFRPRaceEntity>> getByBackground(
			@PathVariable final String background) {
	...
	public List<Resource<WFRPRaceEntity>> save(
			@RequestBody final List<WFRPRaceEntity> entities) {
		List<Resource<WFRPRaceEntity>> resources = new ArrayList<Resource<WFRPRaceEntity>>();
	...
	public List<Resource<WFRPRaceEntity>> save(
			@RequestBody final WFRPRaceEntity entity) {
		if (entity.getSkillChoices() != null
			...
			for (int i = entity.getSkillChoices().size() - 1; i >= 0; i--) {
				...
				if (skillChoices == null) {
					skillChoices = (WFRPSkillChoiceEntity) ((Resource) WFRPSkillChoiceController
							.getInstance()
							.save(entity.getSkillChoices().get(i)).get(0))
									.getContent();
				}
		...
		if (entity.getTalentChoices() != null
			...
			for (int i = entity.getTalentChoices().size() - 1; i >= 0; i--) {
				...
				if (talentChoices == null) {
					talentChoices = (WFRPTalentChoiceEntity) ((Resource) WFRPTalentChoiceController
							.getInstance()
							.save(entity.getTalentChoices().get(i)).get(0))
									.getContent();
				}
