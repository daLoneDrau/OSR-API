...
public class LABLORDAbilityRequirementController {
	 ...
	 */
	private Resource<LABLORDAbilityRequirementEntity> getAbilityRequirementResource(
			final LABLORDAbilityRequirementEntity entity) {
		Resource<LABLORDAbilityRequirementEntity> resource = new Resource<LABLORDAbilityRequirementEntity>(
				entity);
		// link to entity
	...
	public List<Resource<LABLORDAbilityRequirementEntity>> getAll() {
		Iterator<LABLORDAbilityRequirementEntity> iter = repository.findAll()
				.iterator();
		List<Resource<LABLORDAbilityRequirementEntity>> resources = new ArrayList<Resource<LABLORDAbilityRequirementEntity>>();
		while (iter.hasNext()) {
	 ...
	 *         {@link LABLORDAbilityRequirementEntity}>>
	 */
	@RequestMapping(path = "requirement/{requirement}", method = RequestMethod.GET)
	...
	public List<Resource<LABLORDAbilityRequirementEntity>> save(
			@RequestBody final LABLORDAbilityRequirementEntity entity) {
	...
	public List<Resource<LABLORDAbilityRequirementEntity>> save(
			@RequestBody final List<LABLORDAbilityRequirementEntity> entities) {
		List<Resource<LABLORDAbilityRequirementEntity>> resources = new ArrayList<Resource<LABLORDAbilityRequirementEntity>>();
	...
	private void setAbilityIdFromRepository(
		...
		if (memberEntity == null) {
			memberEntity = (LABLORDAbilityEntity) ((Resource) LABLORDAbilityController
					.getInstance().save(
							entity.getAbility())
					.get(0)).getContent();
		}
