...
public class LABLORDEquipmentController {
	...
	public List<Resource<LABLORDEquipmentEntity>> getAll() {
		Iterator<LABLORDEquipmentEntity> iter = repository.findAll()
				.iterator();
		List<Resource<LABLORDEquipmentEntity>> resources = new ArrayList<Resource<LABLORDEquipmentEntity>>();
		while (iter.hasNext()) {
	...
	private Resource<LABLORDEquipmentEntity> getEquipmentResource(
		...
		Resource<LABLORDEquipmentEntity> resource = new Resource<LABLORDEquipmentEntity>(
				entity);
		// link to entity
	...
	public List<Resource<LABLORDEquipmentEntity>> save(
			@RequestBody final LABLORDEquipmentEntity entity) {
				...
				try {
					...
					if (method != null
						...
						if (field.get(entity.getType().get(i)) != null) {
							list = (List<Resource<LABLORDEquipmentTypeEntity>>) method
									.invoke(
											LABLORDEquipmentTypeController
													.getInstance(),
											(String) field.get(
													entity.getType().get(i)));
						}
					}
					if (list == null) {
						try {
	...
	public List<Resource<LABLORDEquipmentEntity>> save(
			@RequestBody final List<LABLORDEquipmentEntity> entities) {
		List<Resource<LABLORDEquipmentEntity>> resources = new ArrayList<Resource<LABLORDEquipmentEntity>>();
	...
	private void setArmorDataIdFromRepository(
		...
		if (memberEntity == null) {
			memberEntity = (LABLORDArmorEntity) ((Resource) LABLORDArmorController
					.getInstance().save(
							entity.getArmorData())
					.get(0)).getContent();
		}
	...
	private void setWeaponDataIdFromRepository(
		...
		if (memberEntity == null) {
			memberEntity = (LABLORDWeaponDataEntity) ((Resource) LABLORDWeaponDataController
					.getInstance().save(
							entity.getWeaponData())
					.get(0)).getContent();
		}
